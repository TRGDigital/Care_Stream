...
export function createApiClient(token: string) {
  return {
    ...
    analytics: {
      ...
      adoptionContext: () => apiFetch<{ enabled: boolean; variables: Array<{ key: string; label: string; value: string }>; role_holders: Array<{ key: string; role: string; candidates: string[] }>; role_names: Record<string, string[]>; show_role_names: boolean; logo_url: string | null; home_name: string; address: string; registered_manager: string; default_approver: string; review_cycle_months: string; version_scheme: string }>('/analytics/gaps/adoption-context', token),

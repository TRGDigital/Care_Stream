...
export function createApiClient(token: string) {
  return {
    ...
    analytics: {
      ...
      safAlignment: (referenceKey: string) => apiFetch<{ statements: Array<{ reference_key: string; name: string; we_statement: string }>; target_policy: { id: string; name: string } | null; alignments: Array<{ focus: string; placement: 'add_under_heading' | 'new_section'; anchor: string; section_title: string; wording: string }>; message: string }>(`/analytics/gaps/${encodeURIComponent(referenceKey)}/saf-alignment`, token, { method: 'POST' }),

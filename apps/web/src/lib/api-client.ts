...
export function createApiClient(token: string) {
  return {
    ...
    analytics: {
      ...
      detectPolicySection: (policyId: string, anchor: string, opts?: { context?: string; granularity?: 'section' | 'sentence' }) => apiFetch<{ section_text: string; found: boolean; debug?: string }>('/analytics/gaps/detect-section', token, { method: 'POST', body: JSON.stringify({ policy_id: policyId, anchor, ...opts }) }),

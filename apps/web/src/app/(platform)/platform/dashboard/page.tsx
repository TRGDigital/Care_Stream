    ...
    <div className="space-y-3" ref={containerRef}>
      ...
      <RefSection icon={Building2} title="Workforce Compliance Register (Enterprise)">
        ...
        <div className="mt-2 space-y-1">
          ...
          <RefRow label="Expiry alerts (V3)" value="services/workforce/credentialExpiry.ts emails a tenant's active admins a digest of credentials expired or expiring within 30 days (references excluded — no expiry). Cron GET /cron/credential-expiry (vercel.json, 0 8 * * *) loops Enterprise tenants; respects the compliance_expiry_alerts email preference. Manual 'Email expiry alerts' button on /workforce → POST /settings/compliance-expiry/send (force=true, bypasses the preference). Uses sendTrainingUpdateEmail." />
          <RefRow label="Supervisions (V4)" value="staff_supervisions table (RLS tenant isolation like the others): one row per session (type supervision|appraisal, held_on, conducted_by, next_due, notes). GET /workforce/supervisions (per-staff latest + next-due + summary), GET/POST /workforce/staff/:userId/supervisions, DELETE /workforce/supervisions/:id. UI: second tab on /workforce (components/admin/workforce/supervisions-view.tsx) + read-only section on /staff/[id]. Status overdue/due_soon(≤30d)/ok/none from next_due." />

    ...
    <div className="space-y-3" ref={containerRef}>
      ...
      <RefSection icon={Building2} title="Workforce Compliance Register (Enterprise)">
        ...
        <div className="mt-2 space-y-1">
          ...
          <RefRow label="api-client"      value="workforce.register() / .staff(userId) / .saveCredential / .deleteCredential / .uploadDocument(userId,type,file) / .downloadDocument / .deleteDocument. settings.sendComplianceExpiryAlerts()." />
          <RefRow label="Roadmap"         value="V1 = register + manual entry (done). V2 = per-credential document uploads + staff-record display (done). V3 = expiry alert digest, cron + manual send (done). Next: mandatory training matrix + Care Certificate, supervisions/appraisals, CQC evidence export + group rollup." />
        </div>
      </RefSection>

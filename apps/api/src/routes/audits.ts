...
auditsRouter.post('/runs/:id/complete', requireAuditAccess, async (req: Request, res: Response) => {
  ...
  // Save the PDF report against the audit and email it to admins. It replaces the plain "audit
  // completed" email below, which is only sent when the home has turned the report email off.
  const reportResult = await generateAndStoreAuditReport(tenantId, run.id, { email: true }).catch(e => { console.error('[audits/complete] report pdf:', e); return 'none' as const })

  ok(res, { run: completed, recommendations, approval_required: false })

  if (reportResult === 'emailed') return
...
auditsRouter.get('/runs/:id/report', requireAuditAccess, async (req: Request, res: Response) => {
  const tenantId = req.user!.tenant_id
  const report = await buildAuditReport(tenantId, String(req.params.id))
  if (!report || !auditTemplateAllowed(req, report.template_id)) return err(res, 'NOT_FOUND', 'Audit run not found', 404)
  ok(res, { report })
})

// GET /audits/runs/:id/report.pdf — the PDF report. ?stored=1 returns the copy saved when it was completed.
auditsRouter.get('/runs/:id/report.pdf', requireAuditAccess, async (req: Request, res: Response) => {
  const tenantId = req.user!.tenant_id
  const run = await (prisma as any).auditRun.findFirst({ where: { id: String(req.params.id), tenant_id: tenantId }, select: { id: true, template_id: true, report_pdf_key: true } })
  if (!run || !auditTemplateAllowed(req, run.template_id)) return err(res, 'NOT_FOUND', 'Audit run not found', 404)
  try {
    let pdf: Buffer, name: string
    if (req.query.stored === '1' && run.report_pdf_key) {
      pdf = await downloadFile(run.report_pdf_key)
      const r = await buildAuditReport(tenantId, run.id)
      name = r ? reportFileName(r) : 'audit-report.pdf'
    } else {
      const out = await renderAuditReportPdf(tenantId, run.id)
      if (!out) return err(res, 'NOT_FOUND', 'Audit run not found', 404)
      pdf = out.pdf; name = reportFileName(out.report)
    }
    res.setHeader('Content-Type', 'application/pdf')
    res.setHeader('Content-Disposition', `attachment; filename="${name}"`)
    res.setHeader('Cache-Control', 'private, no-store')
    res.send(pdf)
  } catch (e: any) { err(res, 'PDF_FAILED', e?.message ?? 'Could not build the PDF report.', 500) }
})

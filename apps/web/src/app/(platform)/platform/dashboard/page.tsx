    ...
    <div className="space-y-3" ref={containerRef}>
      ...
      <RefSection icon={ClipboardList} title="Staff Onboarding Flows (templates → tenant adoption)">
        ...
        <div className="mt-3 space-y-1">
          ...
          <RefRow label="Question source"   value="OnboardingStep.policy_id doubles as a question's SOURCE for answer_question steps: adoption's tailorQuestionsToPolicies already sets it (matched policy); the tenant 'Generate questions from policy' path now also sets policy_id = the source policy (onboarding-modals.tsx). Preview resolves it to the policy name; questions with no policy_id show 'Source not recorded'. It's the tenant's own policy, so no exposure risk." />

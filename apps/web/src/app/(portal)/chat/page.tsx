...
import { ProgrammesView } from '@/components/hub/programmes-view'
...
function ChatPageInner() {
  ...
  const [view,     setView]                            = useState<'chat' | 'induction' | 'training' | 'followup' | 'audits' | 'annual' | 'diplomas' | 'cqc' | 'progress' | 'f2f' | 'supervisions' | 'policies' | 'audit-approvals' | 'actions'>('chat')
  ...
  const [navCounts,    setNavCounts]                    = useState<{ induction: number; training: number; cqc: number; followup: number; annual: number; audits: number; actions: number; programmes: number }>({ induction: 0, training: 0, cqc: 0, followup: 0, annual: 0, audits: 0, actions: 0, programmes: 0 })
  ...
  useEffect(() => {
    ...
    if (v === 'induction' || v === 'training' || v === 'annual' || v === 'diplomas' || v === 'followup' || v === 'audits' || v === 'cqc' || v === 'progress' || v === 'policies' || v === 'audit-approvals' || v === 'actions') {
  ...
  useEffect(() => {
    ...
    createApiClient(session.accessToken).me.counts()
      .then(c => { const v = { induction: c.induction, training: c.training, cqc: c.cqc, followup: c.followup, annual: c.annual, audits: c.audits, actions: c.actions ?? 0, programmes: c.programmes ?? 0 }; setNavCounts(v); try { localStorage.setItem(`cs_counts_${userId}`, JSON.stringify(v)) } catch { /* ignore */ } })
      .catch(() => {})
      ...
      <aside className={`w-64 flex-shrink-0 flex-col border-r border-gray-200 bg-white overflow-y-auto [@media(min-width:768px)_and_(min-height:800px)]:overflow-hidden md:static md:z-auto md:flex md:shadow-none ${navOpen ? 'fixed inset-y-0 left-0 z-50 flex shadow-2xl' : 'hidden'}`}>
        ...
        <div className="flex-shrink-0 p-3 space-y-0.5">
          ...
          {navCounts.programmes > 0 && (
            <button
              onClick={() => setView('diplomas')}
              className={`flex w-full items-center gap-2 rounded-md px-3 py-2 text-sm font-medium transition-colors ${view === 'diplomas' ? 'bg-teal/10 text-teal' : 'text-neutral-mid hover:bg-neutral-light hover:text-neutral-dark'}`}
            >
              <Award size={15} />
              My Diplomas
              <NavBadge count={navCounts.programmes} className="bg-teal-dark" />
            </button>
          )}
          <button
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        {view === 'annual' && session?.accessToken && (
          <AnnualTrainingView token={session.accessToken} userId={userId} secondLang={secondLang} reviewer={isReviewer} onTalkToPolicy={talkToPolicy} onChange={() => {
            createApiClient(session.accessToken).me.counts().then(c => setNavCounts({ induction: c.induction, training: c.training, cqc: c.cqc, followup: c.followup, annual: c.annual, audits: c.audits, actions: c.actions ?? 0, programmes: c.programmes ?? 0 })).catch(() => {})
          }} />
        )}

        {/* My Diplomas view — programmes over the same annual-training records */}
        {view === 'diplomas' && session?.accessToken && (
          <ProgrammesView token={session.accessToken} secondLang={secondLang} onTalkToPolicy={talkToPolicy} onChange={() => {
            createApiClient(session.accessToken).me.counts().then(c => setNavCounts({ induction: c.induction, training: c.training, cqc: c.cqc, followup: c.followup, annual: c.annual, audits: c.audits, actions: c.actions ?? 0, programmes: c.programmes ?? 0 })).catch(() => {})
          }} />
        )}
...
function TrainingView({ token, userId, secondLang = null }: { token: string; userId: string; secondLang?: { name: string } | null }) {
  ...
  // Lazily-fetched second-language copy of the enrollments (by id) so a flipped
  // question can show its translated text/options. Answers use language-independent
  ...
  function toggleQ(enrollmentId: string, questionId: string, setName?: string) {
    if (!langByQ[questionId]) { ensureSecond(); api.me.recordLanguageSwitch({ area: 'training', set_ref: enrollmentId, set_name: setName }).catch(() => {}) }
    setLangByQ(prev => ({ ...prev, [questionId]: !prev[questionId] }))
  }
  // The 2nd-language copy of a question (matched by id) when the learner flipped it.
  function qView(enrollmentId: string, question: any) {
    if (!langByQ[question.id]) return question
    const q2 = (secondEnr?.[enrollmentId]?.module?.questions ?? []).find((q: any) => q.id === question.id)
    return q2 ?? question
  }
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <div className="space-y-6">
          {enrollments.map(enrollment => {
            // Base (first-language) question list; individual questions flip to the
            // 2nd language on their own toggle. Progress/answers stay language-independent.
              ...
              <div key={enrollment.id} className="rounded-xl border border-gray-200 bg-white shadow-sm">
                ...
                <div className="divide-y divide-gray-50 px-5">
                  {questions.map((question, qi) => {
                    ...
                    // Per-question second-language aid: this individual question flips to
                    // the 2nd language only while its own toggle is on.
                    const qv             = qView(enrollment.id, question)
                    const opts: string[] = Array.isArray(qv.options) ? qv.options : []
                    ...
                    return (
                      <div key={question.id} className="py-4">
                        <div className="mb-3 flex items-start justify-between gap-2">
                          ...
                          {secondLang && (
                            <button
                              onClick={() => toggleQ(enrollment.id, question.id, enrollment.module.name)}
                              disabled={loadingLang && !langByQ[question.id]}
                              title={langByQ[question.id] ? `Showing in ${secondLang.name}. Tap to read in your first language.` : `Read this question in ${secondLang.name}`}
                              className={`inline-flex shrink-0 items-center gap-1 rounded-lg border px-2 py-1 text-xs font-medium disabled:opacity-50 ${langByQ[question.id] ? 'border-teal bg-teal/10 text-teal' : 'border-gray-200 text-neutral-mid hover:border-teal/40 hover:text-teal'}`}
                            >
                              <Globe size={12} /> {langByQ[question.id] ? '1st language' : secondLang.name}
                            </button>
                          )}
                        </div>
...
function InductionView({ token, userId, onSavedChange, onTalkToPolicy, secondLang = null }: { token: string; userId: string; onSavedChange?: () => void; onTalkToPolicy?: (policyId: string, title: string) => void; secondLang?: { name: string } | null }) {
  ...
  function toggleStep(enrollmentId: string, stepId: string, setName?: string) {
    if (!langByStep[stepId]) { ensureSecond(); api.me.recordLanguageSwitch({ area: 'induction', set_ref: enrollmentId, set_name: setName }).catch(() => {}) }
    setLangByStep(prev => ({ ...prev, [stepId]: !prev[stepId] }))
  }
  // The step to display: the 2nd-language copy (same index in the flow) when the
  // learner has flipped just this step; otherwise the first-language step.
  function stepView(enrollmentId: string, i: number, step: any) {
    return langByStep[step.id] ? (secondEnr?.[enrollmentId]?.steps?.[i] ?? step) : step
  }
    ...
    <div className="flex-1 overflow-y-auto px-4 py-6">
              ...
              <div key={e.enrollment_id} className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
                ...
                <div className="divide-y divide-gray-50 border-t border-gray-100 px-5">
                  {e.steps.map((step: any, i: number) => {
                    ...
                    const vstep       = stepView(e.enrollment_id, i, step)   // 2nd language for just this step when flipped
                    return (
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="mt-0.5 flex flex-wrap items-center gap-2">
                              ...
                              {secondLang && !isCompleted && !isLocked && (
                                <button
                                  type="button"
                                  onClick={() => toggleStep(e.enrollment_id, step.id, e.flow_name)}
                                  disabled={!langByStep[step.id] && loadingLang && !secondEnr}
                                  title={langByStep[step.id] ? 'Back to your first language' : `Show this step in ${secondLang.name}`}
                                  className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[11px] font-medium ${langByStep[step.id] ? 'border-teal bg-teal/10 text-teal' : 'border-gray-200 text-neutral-mid hover:border-teal/40 hover:text-teal'}`}
                                >
                                  <Globe size={11} /> {(!langByStep[step.id] && loadingLang && !secondEnr) ? 'Loading…' : langByStep[step.id] ? '1st language' : secondLang.name}
                                </button>
                              )}
                            </div>
      ...
      {viewer && (
        <PolicyViewer
          ...
          lang={langByStep[viewer.stepId] ? '2' : undefined}

...
type Delivery = {
  ...
  attempts?:   number
  question:    { domain: string; model_answer?: string | null; job_role?: string | null; is_standard?: boolean }
...
// and the standard CQC set (the four themes inspectors raise in almost every
// inspection) from both.
function RoleTag({ jobRole, isStandard }: { jobRole?: string | null; isStandard?: boolean }) {
  if (isStandard) {
    return (
      <span className="inline-block rounded-full border border-teal/30 bg-teal-light/40 px-2 py-0.5 text-[10px] font-semibold text-teal">
        CQC Standard question
      </span>
    )
  }
  return (
                          ...
                          <div key={d.id} className="border-b border-gray-50 last:border-0">
                            <button onClick={() => setExpanded(expanded === d.id ? null : d.id)} className="w-full flex items-start justify-between gap-3 py-3.5 text-left">
                              <div className="flex-1 min-w-0">
                                <RoleTag jobRole={d.question.job_role} isStandard={d.question.is_standard} />

...
import { Loader2, ListChecks, Check, Clock, CalendarDays, CheckCircle2 } from 'lucide-react'

type Action = { id: string; description: string; priority: string; due_date: string | null; status: string; done_at: string | null; run_id: string; audit_name: string }
...
export function MyActionsView({ token, userId, onChange }: { token: string; userId?: string; onChange?: () => void }) {
  const cacheKey = `hub-my-actions-${userId ?? 'me'}`
  const cached = persistentCache.get<Action[]>(cacheKey)
  const [actions, setActions] = useState<Action[]>(cached ?? [])
  const [loading, setLoading] = useState(!cached)
  const [busy, setBusy]       = useState<string | null>(null)

  function apply(list: Action[]) { setActions(list); persistentCache.set(cacheKey, list) }

  useEffect(() => {
    createApiClient(token).me.actions()
      .then(r => apply(r.actions))
      .catch(() => {})
      .finally(() => setLoading(false))
  }, [token]) // eslint-disable-line react-hooks/exhaustive-deps

  async function setStatus(id: string, status: 'open' | 'in_progress' | 'done') {
    setBusy(id)
    try {
      const r = await createApiClient(token).me.setActionStatus(id, status)
      apply(r.actions); onChange?.()
    } catch { /* keep current on failure */ } finally { setBusy(null) }
  }
  ...
  const outstanding = actions.filter(a => a.status !== 'done')
                ...
                return (
                  <div key={a.id} className="rounded-lg border border-gray-200 bg-white p-4">
                    ...
                    <div className="mt-3 flex items-center gap-2">
                      {a.status === 'open' ? (
                        <button onClick={() => setStatus(a.id, 'in_progress')} disabled={busy === a.id}
                      ...
                      <button onClick={() => setStatus(a.id, 'done')} disabled={busy === a.id}
                  ...
                  {done.map(a => (
                    <li key={a.id} className="flex items-start justify-between gap-3 rounded-lg border border-gray-100 bg-white px-4 py-3">
                      <div className="min-w-0">
                        ...
                        <p className="mt-0.5 text-xs text-neutral-mid">{a.audit_name}{a.done_at ? ` · done ${dateLabel(a.done_at)}` : ''}</p>
                      </div>
                      <button onClick={() => setStatus(a.id, 'open')} disabled={busy === a.id}

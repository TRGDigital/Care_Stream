    ...
    <div className="space-y-3" ref={containerRef}>
      ...
      <RefSection icon={Globe} title="Language Detection & Multilingual Responses">
        ...
        <div className="mt-3 space-y-1">
          ...
          <RefRow label="Translation overrides" value="Human-verified overrides. translation_overrides table (per tenant+lang+source_hash). Permitted staff (users.can_suggest_translations) submit via POST /me/translation-suggestion; stored pending unless tenant.translation_suggestions_auto_approve. overrides.ts approvedOverrideMap (60s TTL) overlays approved suggested_text over machine output in translate.ts (questions + texts), AFTER cache read so the cache keeps the machine value. Admin queue: /translation-suggestions (Translation Review page). Platform oversight of all care settings: /admin/translation-changes (Translation Changes tab). Hub entry point: TakeModule suggest control (source_questions English snapshot returned by /me/annual-training/:id when lang!=eng and can_suggest)." />

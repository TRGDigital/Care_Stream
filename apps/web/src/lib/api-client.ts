...
export function createApiClient(token: string) {
  return {
    ...
    policies: {
      ...
      minorEdit: (id: string, body: { old_text: string; new_text: string }) =>
        apiFetch<{ version: string; propagated: boolean }>(`/policies/${id}/minor-edit`, token, { method: 'POST', body: JSON.stringify(body) }),
    ...
    cqcQuestions: {
      ...
      generateForStaff: (data: { user_ids: string[]; per_role?: number; core?: number; standard?: number; domains?: string[] }) =>
        apiFetch<{ delivered: number; roles: Array<{ role: string; recipients: number; questions: number }>; core_questions: number; standard_questions: number; credits_used: number }>('/cqc-questions/generate-for-staff', token, { method: 'POST', body: JSON.stringify(data) }),
      deactivate: (id: string) =>
    ...
    audits: {
      ...
      actionsSummary: () => apiFetch<{
        ...
        actions: Array<{ id: string; description: string; assigned_to: string; status: string; priority: string; due_date: string | null; done_at: string | null; audit_name: string; overdue: boolean }>
      }>('/audits/actions/summary', token),
      ...
      updateAuditAction: (actionId: string, patch: { description?: string; priority?: string; due_date?: string | null; assigned_to?: string | null; is_external?: boolean; external_name?: string | null; status?: string }) => apiFetch<{ updated: boolean }>(`/audits/actions/${encodeURIComponent(actionId)}`, token, { method: 'PATCH', body: JSON.stringify(patch) }),
    ...
    me: {
      ...
      policyExternalSendContext: (policyId: string) => apiFetch<{ policy_name: string; org_name: string; changes: number; reviewer_name: string; reviewer_email: string; sent: boolean; sent_at: string | null; link_expired: boolean; link_ttl_days: number; subject: string; message: string }>(`/me/policy-approvals/${encodeURIComponent(policyId)}/external`, token),
      sendPolicyExternal: (policyId: string, name: string, email: string) => apiFetch<{ sent: boolean }>(`/me/policy-approvals/${encodeURIComponent(policyId)}/send-external`, token, { method: 'POST', body: JSON.stringify({ name, email }) }),
    ...
    analytics: {
      ...
      detectPolicySection: (policyId: string, anchor: string, opts?: { context?: string; granularity?: 'section' | 'sentence' }) => apiFetch<{ section_text: string; found: boolean; reason?: string | null }>('/analytics/gaps/detect-section', token, { method: 'POST', body: JSON.stringify({ policy_id: policyId, anchor, ...opts }) }),
      ...
      safAlignment: (referenceKey: string, policyId: string, force = false) => apiFetch<{ statements: Array<{ reference_key: string; name: string; we_statement: string }>; target_policy: { id: string; name: string } | null; alignments: Array<{ focus: string; placement: 'amend' | 'add_under_heading' | 'new_section'; anchor: string; section_title: string; wording: string }>; message: string }>(`/analytics/gaps/${encodeURIComponent(referenceKey)}/saf-alignment`, token, { method: 'POST', body: JSON.stringify({ policy_id: policyId, force }) }),
      // CQC wording alignment as its own analysis (per policy, re-run over the whole library).
      wordingAlignmentStart: () => apiFetch<{ total: number }>('/analytics/wording-alignment/start', token, { method: 'POST' }),
      wordingAlignmentBatch: () => apiFetch<{ done: number; analysed: number; total: number; remaining: number }>('/analytics/wording-alignment/batch', token, { method: 'POST' }),
      wordingAlignment: () => apiFetch<{ policies: Array<{ policy_id: string; policy_name: string; statements: Array<{ reference_key: string; name: string; we_statement: string }>; alignments: Array<{ focus: string; placement: 'amend' | 'add_under_heading' | 'new_section'; anchor: string; section_title: string; wording: string }>; message: string; has_suggestions: boolean }>; analysed: number; total_suggestions: number; analysed_at: string | null }>('/analytics/wording-alignment', token),
      ...
      // Coverage suggestion decisions: 'ignored' or 'new_section', so a later
      // analysis stops proposing something already dealt with.
      setCoverageDecision: (referenceKey: string, body: { requirement: string; decision: 'ignored' | 'new_section'; policy_id?: string | null; section_title?: string | null }) =>
        apiFetch<{ decisions: Array<{ requirement: string; decision: string; section_title: string | null; policy_id: string | null; decided_at: string }> }>(`/analytics/gaps/${encodeURIComponent(referenceKey)}/decision`, token, { method: 'POST', body: JSON.stringify(body) }),
      clearCoverageDecision: (referenceKey: string, requirement: string, policyId?: string | null) =>
        apiFetch<{ decisions: Array<{ requirement: string; decision: string; section_title: string | null; policy_id: string | null; decided_at: string }> }>(`/analytics/gaps/${encodeURIComponent(referenceKey)}/decision?requirement=${encodeURIComponent(requirement)}${policyId ? `&policy_id=${encodeURIComponent(policyId)}` : ''}`, token, { method: 'DELETE' }),
      adoptSuggestion: (body: { policy_id: string; reference_key: string; requirement: string; placement: string; old_text: string; new_text: string; section_title?: string }) =>
        apiFetch<{ applied: boolean; pending: number; document_id: string; change_id: string; placed_at_end?: boolean }>('/analytics/gaps/adopt', token, { method: 'POST', body: JSON.stringify(body) }),
      ...
      policyApprovalsOverview: () => apiFetch<{ documents: Array<{ policy_id: string; name: string; version: string; approval_status: string; published_at: string | null; updated_at: string | null; pending: number; external_name: string; content_propagated: boolean; trail: Array<{ stage: string; decision: string; approver_name: string; created_at: string; comment: string }> }>; summary: { total: number; published: number; in_progress: number }; health: { policies_total: number; coverage_score: number | null; regs_total: number; regs_covered: number; regs_partial: number; regs_gap: number; review_tracked: number; review_due: number } }>('/analytics/gaps/approvals-overview', token),
      ...
      removePolicyChangeWording: (changeId: string) => apiFetch<{ pending: number }>(`/analytics/gaps/policy-change/${encodeURIComponent(changeId)}`, token, { method: 'PATCH', body: JSON.stringify({ remove: true }) }),
      ...
      repropagatePolicyDocument: (policyId: string) => apiFetch<{ ok: boolean }>(`/analytics/gaps/policy-document/${encodeURIComponent(policyId)}/repropagate`, token, { method: 'POST' }),
      policyApprovals: (policyId: string) => apiFetch<{ status: string; external_name: string; external_email: string; external_token: string | null; external_sent_at: string | null; stage_since: string | null; link_expired: boolean; default_external_name: string; default_external_email: string; approvals: Array<{ id: string; stage: string; decision: string; approver_name: string; approver_email: string; comment: string; created_at: string }> }>(`/analytics/gaps/policy-approvals/${encodeURIComponent(policyId)}`, token),
      remindApproval: (policyId: string) => apiFetch<{ reminded: boolean }>(`/analytics/gaps/policy-document/${encodeURIComponent(policyId)}/remind`, token, { method: 'POST' }),
      policyVersions: (policyId: string) => apiFetch<{ versions: Array<{ id: string; version: string; change_count: number; published_by: string; published_at: string }> }>(`/analytics/gaps/policy-document/${encodeURIComponent(policyId)}/versions`, token),
      policyVersion: (versionId: string) => apiFetch<{ version: string; content: string; published_by: string; published_at: string }>(`/analytics/gaps/policy-version/${encodeURIComponent(versionId)}`, token),
      ...
      revokeExternalLink: (policyId: string) => apiFetch<{ revoked: boolean }>(`/analytics/gaps/policy-document/${encodeURIComponent(policyId)}/external-revoke`, token, { method: 'POST' }),
      reissueExternalLink: (policyId: string) => apiFetch<{ token: string }>(`/analytics/gaps/policy-document/${encodeURIComponent(policyId)}/external-reissue`, token, { method: 'POST' }),
      engagement: () => apiFetch<{

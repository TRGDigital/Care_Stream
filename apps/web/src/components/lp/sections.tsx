      ...
      <div className="mx-auto max-w-content px-6">
        <div className="grid items-center gap-12 lg:grid-cols-2">
          <div>
            ...
            <h2 className="mb-6 text-3xl font-extrabold leading-tight text-neutral-dark md:text-4xl">{withAccents(data.headline)}</h2>
  ...
  return (
    <section className="bg-cream-warm py-20">
      <div className="mx-auto max-w-content px-6">
        ...
        <h2 className="mb-4 text-3xl font-extrabold leading-tight text-neutral-dark md:text-4xl">{withAccents(data.headline)}</h2>
        {data.subheadline && <p className="mb-12 max-w-2xl text-lg leading-relaxed text-neutral-mid">{data.subheadline}</p>}
        <div className="grid gap-6 md:grid-cols-3">
          {data.steps.map(s => (
            <div key={s.number} className="rounded-2xl border border-cream-line bg-white p-7 shadow-card">
              <span className="lp-num mb-2 block text-6xl leading-none text-teal/25">{s.number}</span>
      ...
      <div className="mx-auto max-w-content px-6">
        ...
        <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {data.items.map(item => {
            ...
            return (
              <div key={item.title} className="rounded-2xl border border-cream-line bg-white p-6 shadow-card">
...
export function LpSocialProof({ data }: { data: NonNullable<LpContent['socialProof']> }) {
  ...
  return (
    <section className="bg-cream-warm py-20">
      <div className="mx-auto max-w-content px-6">
        {data.headline && <h2 className="mb-10 text-3xl font-extrabold leading-tight text-neutral-dark md:text-4xl">{withAccents(data.headline)}</h2>}
        ...
        {(data.testimonials?.length ?? 0) > 0 && (
          <div className="grid gap-6 md:grid-cols-3">
            {data.testimonials!.map(t => (
              <div key={t.name} className="rounded-2xl border border-cream-line bg-white p-6 shadow-card">

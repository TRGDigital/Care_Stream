...
trainingRouter.post('/catalogue/generate', requireAdmin, async (req: Request, res: Response) => {
  ...
  try {
    ...
    const data = {
      ...
      // regenerating preserves any existing image. Changing the image is a separate action.
...
// GET /training/modules/:id/preview — normalised read-only content of the TENANT's own
// module (lesson sections + questions with answers), for an admin preview. Same shape as
// the standard-module viewer, so the shared stepped player renders it.
trainingRouter.get('/modules/:id/preview', requireAdmin, async (req: Request, res: Response) => {
  const tenantId = (req as any).user.tenant_id
  const module = await (prisma as any).trainingModule.findFirst({ where: { id: req.params.id, tenant_id: tenantId } })
  if (!module) { err(res, 'NOT_FOUND', 'Module not found', 404); return }
  const lc = (module.learning_content ?? {}) as any
...
async function ensureAnnualTopicsAsAdhoc(tenantId: string): Promise<void> {
  ...
  await (prisma as any).trainingModule.createMany({ data: rows, skipDuplicates: true })
}
...
trainingRouter.get('/modules', async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Surface every standard/annual training topic name here too, as an empty
    // generatable adhoc module (deduped), so tenants can build adhoc versions of them.
    await ensureAnnualTopicsAsAdhoc(tenantId)

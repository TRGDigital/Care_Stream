...
function AuditList({ api, onOpen }: { api: ReturnType<typeof createApiClient>; onOpen: (runId: string) => void }) {
  ...
  function load() {
    ...
    Promise.all([api.audits.templates(), api.audits.runs()])
      .then(([t, r]) => { setTemplates(t.templates ?? []); setRuns(r.runs ?? []) })
      .catch(() => {})
  ...
  async function start(templateId: string) {
    ...
    try {
      ...
      const { run } = await api.audits.createRun({ template_id: templateId, audit_month: month })
      onOpen(run.id)
            ...
            <div className="space-y-1.5">
              {completed.map(r => (
                <button key={r.id} onClick={() => onOpen(r.id)} className="flex w-full items-center gap-2 rounded-lg px-2 py-2 text-left hover:bg-neutral-light">
                  ...
                  <span className="min-w-0 flex-1 truncate text-sm text-neutral-dark">{r.template?.name ?? 'Audit'}</span>

...
// ─── Read-only viewer for a STANDARD module — stepped, exactly as staff see it ──
function ViewStandardModule({ api, id, name, onBack }: { api: ReturnType<typeof createApiClient>; id: string; name: string; onBack: () => void }) {
  ...
  useEffect(() => {
    api.training.standardModuleFull(id)
      .then(d => { setM(d.module); setStep(0) })
          ...
          {cur?.type === 'overview' && (
            <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-card">
              ...
              {m.key_points?.length > 0 && (
                <ul className="mt-4 space-y-1.5">{m.key_points.map((p: string, i: number) => <li key={i} className="flex gap-2 text-sm text-neutral-dark"><span className="text-teal">✓</span>{p}</li>)}</ul>
              )}
            </div>
          )}
          ...
          {cur?.type === 'question' && questions[cur.i] && (
            <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-card">
              ...
              <ul className="mt-3 space-y-1.5">
                {questions[cur.i].options.map((opt: string, oi: number) => (
            ...
            <div className="rounded-2xl border border-gray-100 bg-white p-8 text-center shadow-card">
              ...
              {m.standards?.length > 0 && (
                <div className="mt-4">
                  ...
                  <div className="flex flex-wrap justify-center gap-2">{m.standards.map((s: string, i: number) => <span key={i} className="rounded-full bg-gray-100 px-2.5 py-1 text-xs text-neutral-mid">{s}</span>)}</div>
                </div>
              )}
            </div>
          )}

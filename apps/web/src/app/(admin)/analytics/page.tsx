...
export default function AnalyticsPage() {
  ...
  // Training-only tenants: only training analytics apply — the policy/audit/CQC
  // surfaces don't exist for them, so every other tab is hidden.
  ...
  // Training-only tenants land straight on (and can only see) the Training tab.
  useEffect(() => { if (trainingOnly) setTab('training') }, [trainingOnly])
    ...
    <div>
      ...
      <div className="sticky -top-6 z-30 -mx-6 -mt-6 mb-6 bg-neutral-light px-6 pt-6 shadow-sm">
        ...
        <div className="no-scrollbar flex gap-1 overflow-x-auto border-b border-gray-200">
          {TABS.filter(t => trainingOnly ? t.id === 'training' : (t.id !== 'policies' || policiesOverview != null)).map(t => {

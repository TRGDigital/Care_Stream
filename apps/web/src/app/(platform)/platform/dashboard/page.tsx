      ...
      <ChannelCard icon={MessageSquare} title="Web Chat (Staff Portal)" color="bg-teal">
        ...
        <div className="mt-1 space-y-1.5">
          ...
          <div className="rounded-lg border border-purple-200 bg-purple-50 px-3 py-2 text-xs text-purple-800">
            <strong>Full-policy requests (reworked Aug 2026):</strong> <span className="text-neutral-dark">&ldquo;Send me the full policy for X&rdquo; is intercepted BEFORE the RAG summary path (<code className="font-mono">services/rag/query.ts</code> → full-policy branch). The policy is resolved by <em>fuzzy name matching</em> against every active policy — tolerant of dash variants (&ldquo;-&rdquo; typed for &ldquo;–&rdquo;), quotes and filler words — with a conversation-history fallback. English requests are answered with the hub&rsquo;s own cached formatted HTML (<code className="font-mono">getEnglishPolicyHtml</code>, the same render as the full-copy view, regenerated on every republish) so chat always sends the CURRENT version with hub-identical formatting; non-English requests format via Prompt B. If the name can&rsquo;t be confidently matched, the chat replies &ldquo;did you mean …?&rdquo; with the closest policy names as tappable suggestions — it never presents retrieved excerpts as a full policy (Prompt A now also forbids <code className="font-mono">&lt;pre&gt;</code> dumps outright). Track in the queries table: <code className="font-mono">intent_type = 'full_policy'</code> (with <code className="font-mono">no_match = true</code> for did-you-mean replies); a full-policy request recorded as <code className="font-mono">'summary'</code> would indicate a regression (cf. REF-672D5B, fixed).</span>
          </div>
        </div>
      </ChannelCard>
    ...
    <div className="space-y-3" ref={containerRef}>
      ...
      <RefSection icon={BookOpen} title="Regulations Database">
        ...
        <div className="mt-3 rounded-lg border border-amber-200 bg-amber-50/60 p-3">
          ...
          <div className="mt-2 space-y-1">
            ...
            <RefRow label="authoritative_requirements" value="Vetted statement of what the actual standard/legislation requires, provisions + sources cited. Generate-a-draft button (Sonnet) → review against sources → save. The primary grounding for the required_elements checklist." />
            <RefRow label="required_elements"      value="Concrete per-line checklist a policy is scored against. Generate grounds it in authoritative_requirements when set (else the reg's summary/context). Spine of the tenant 'what to add' deep-dive; model-derived fallback bound to the description when empty." />
      ...
      <RefSection icon={BookOpen} title="Gaps Guided Pipeline (/gaps sequencing)">
        ...
        <div className="mt-2 space-y-1">
          ...
          <RefRow label="Pre-run guardrail"    value="EVERY run press shows a dialog: strong amber warning when pending_changes > 0 (run reads published version, N adopted changes will not be seen — Run anyway / Go to Policies), lighter teal reminder of the run → adopt → publish cycle otherwise" />
          <RefRow label="Publish ticker"       value="Single amber 'N policies to publish' chip in the pipeline overview strip (pending_policies = distinct docs with unpublished changes), linking to /policies; pipeline polled every 60s while /gaps is open + persistentCache for instant paint" />
          <RefRow label="Staleness sources"    value="stale_policy_count = policies with changes PUBLISHED since the run + policies UPLOADED since the run (a run that never saw a new policy is stale too)" />
          <RefRow label="Info hovers"          value="An (i) next to every step status pill explains the run → adopt → approve and publish cycle on hover" />
          <RefRow label="Resumable runs"       value="GET /analytics/gaps/run-state (coverage/consistency/wording: total/analysed/remaining from stored batch state, no AI cost). Interrupted runs show an amber Resume banner in their section; Resume continues the batch loop WITHOUT the destructive start endpoint, so finished items never re-bill. Lint is single-shot, no resume needed." />
          <RefRow label="Post-publish nudge"   value="/policies shows a green 'Published — re-run your Gaps analyses' banner (Go to Gaps) after any publish from the review modal" />
          <RefRow label="Error states"         value="Every /gaps section loader surfaces a visible 'Couldn't load — Retry' box instead of silently rendering empty on API failures" />
      ...
      <RefSection icon={ClipboardList} title="Staff Onboarding Flows (templates → tenant adoption)">
        ...
        <div className="mt-3 space-y-1">
          ...
          <RefRow label="Question feedback" value="Feedback loop for ready-made questions. In FlowPreview, ready-made (source_flow_id) questions show a Yes/No rating → POST /onboarding/question-feedback {flow_id, step_id, question, rating: good|not_relevant|not_good, comment}. Server resolves source_flow_id (template) + policy_section and stores onboarding_question_feedback. Platform: /platform/onboarding-flows → Feedback tab (GET /admin/onboarding-templates/feedback?rating=) — per-template rollup ranked by negative count + individual items (question, comment, tenant, section). Manual training signal to refine/re-draft template questions; long-term as volume grows." />

      ...
      {tab === 'bank' && (
        <>
          <HelpAccordion title="How CQC Staff Prep works">
            <p><strong className="text-neutral-dark">Question Bank</strong> — 21 pre-loaded CQC inspector-style questions are organised across five domains: Safe, Effective, Caring, Responsive, and Well-led. Each question shows its <strong className="text-neutral-dark">model answer</strong> and a tracking line (how many staff it&apos;s been assigned to, how many answered, and their average score). Add your own with <strong className="text-neutral-dark">Add question</strong>, or use <strong className="text-neutral-dark">Generate with AI</strong> to create a whole batch at once (AI tokens per question generated).</p>
            <p><strong className="text-neutral-dark">Send role-matched questions (recommended)</strong> — pick the staff first; they&apos;re grouped by job role. The AI then generates a fresh set of questions for each role — the chef gets food safety and allergens, nurses get medicines and clinical care, domestics get COSHH and infection control — plus a small <strong className="text-neutral-dark">core set</strong> everyone gets (safeguarding, whistleblowing, fire), because inspectors ask those of every member of staff.</p>
            <p><strong className="text-neutral-dark">Choosing domains</strong> — use the domain pills to focus a round on one CQC domain (e.g. everyone practises <em>Safe</em> this week, each in their own role), a few, or all five spread evenly. One AI tokens per question generated; the estimate shows before you send, and each person gets a single notification for the round.</p>
